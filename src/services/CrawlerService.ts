...
class CrawlerService {
    ...
    public async fetchProcessDetails(processNumber: string): Promise<ProcessDetails | null> {
        try {
            await this.init();
            const page: Page = await this.browser.newPage();
            Logger.info('Nova aba aberta');
            await page.goto('https://pje-consulta-publica.tjmg.jus.br/pje/ConsultaPublica/listView.seam');
            ...
            const newPagePromise: Promise<Page> = new Promise((resolve, reject) => {
                this.browser.once('targetcreated', async (target: Target) => {
                    ...
                    newPage ? resolve(newPage) : reject(new Error('Nova página não encontrada'));
                });
            });
